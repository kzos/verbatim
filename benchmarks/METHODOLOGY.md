...
This methodology is frozen and committed before the first head-to-head comparative row exists, with a target of 2026-09-30. A local commit date is forgeable, so the freeze is proven only by a push to the public repository or by an external timestamp of the commit recorded in a `docs/decisions/` record. This repository is public, so the push is that proof: the commit introducing this document carries a date GitHub records independently of any local clock, and it predates every comparative row because none exists. The hash is recorded in `docs/decisions/0001-freeze-the-benchmark-methodology.md`.
