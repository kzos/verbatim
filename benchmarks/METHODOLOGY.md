...
This methodology is frozen and committed before the first head-to-head comparative row exists, with a target of 2026-09-30. A local commit date is forgeable, so the freeze is proven only by a push to the public repository or by an external timestamp of the commit recorded in a `docs/decisions/` record. Neither proof exists yet. This repository is private and its history was restarted on 2026-09-10, so the freeze is **unproven** until either a push to a public remote or a `docs/decisions/` record carrying an external timestamp. This memo says so rather than letting a local commit date stand in for it, and `docs/decisions/0001-freeze-the-benchmark-methodology.md` is the record that becomes the proof.
