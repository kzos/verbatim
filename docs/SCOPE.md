...
3. **Batch invariance.** A session's transcript and timestamps are bit-identical regardless of concurrency and batch composition on the graph path. A CI gate runs the same corpus at concurrency `1 / 32 / maximum` and diffs outputs: `verbatim-bench invariance --endpoint ws://host:port/v1/stream --manifest <corpus> --max <the served ceiling>`, which compares the four levels against each other rather than against any reference, names every diverging stream with the first differing word or timing on both sides, and exits non-zero on divergence. It runs in CI against `--pipeline fake`, which proves the gate works end to end and proves nothing about any model; the run that speaks to this clause is the one against a real pipeline on owned hardware. Any padding artefact found, including the `#12840` class, is fixed upstream in NeMo first.
