...
OUT = os.environ.get("PROBE_OUT", "probe-output/streaming_divergence_controlled.json")
...
def stream(audios, pad_to=None):
    ...
    pad_to zero-pads every row to one common length BEFORE the buffer sees it, so the number of chunks,
    the step at which the buffer empties, and the value of keep_all_outputs are all identical between the
    alone arm and the batched arm. Without that control, a short utterance streamed beside long neighbours
    receives extra trailing chunks that it does not receive when streamed alone, and its tail decodes
    further. That is a real batch-composition effect, but it is a boundary-handling one, not the numerical
    perturbation this experiment is trying to isolate."""
...
for i in range(N_TARGETS):
    ...
    common = max(len(a) for a in [target] + neighbours)
